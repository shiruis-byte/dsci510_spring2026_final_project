...
def get_unemployment_data(api_key: str,
    ...
    """
    Fetch monthly state-level unemployment rates from the BLS API.

    Parameters:
        api_key   : your BLS registration key
        start_year: start of the time range (default 2019)
        end_year  : end of the time range (default 2024)

    Returns:
        list of dicts, each with keys: state, year, month, unemployment_rate
    """

    # BLS series IDs for each state (LAUST + FIPS code + 0000000000003)
    state_series = {
    ...
    # API allows max 50 series per request with a registered key
...
def get_top10_states(records: list) -> list:
    """
    Given a list of records from get_unemployment_data(),
    return the 10 state names with the highest average unemployment rate.
    """
...
def plot_unemployment_trends(records: list, top10_states: list) -> None:
    """
    Plot monthly unemployment rate trends for the top 10 states.

    Parameters:
        records      : list of records from get_unemployment_data()
        top10_states : list of state names from get_top10_states()
    """
